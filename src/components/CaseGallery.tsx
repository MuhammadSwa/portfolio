...
export default function CaseGallery(props: Props) {
  ...
  return (
    <div>
      ...
      <div class="mb-8 space-y-4">
        ...
        <div class="flex items-center justify-between">
          <p class="text-slate-400 text-sm">
            Showing <span class="font-semibold text-slate-200">{filteredCases().length}</span> cases
          </p>
      ...
      {/* Cases Grid */}
      <Show when={filteredCases().length > 0} fallback={
        <div class="text-center py-16">
          <div class="text-6xl mb-4">🔍</div>
          <h3 class="text-xl font-semibold text-slate-200 mb-2">No cases found</h3>
          ...
          <For each={filteredCases()}>
            {(caseItem) => (
              <Show when={viewMode() === 'grid'} fallback={
                <ListCard case={caseItem} formatDate={formatDate} />
              }>
  ...
  return (
    ...
    >
      ...
      <div class="aspect-video bg-gradient-to-br from-slate-900 to-slate-800 relative overflow-hidden">
        <Show when={props.case.images.length > 0} fallback={
      ...
      <div class="p-5">
        <div class="flex items-center gap-2 mb-3">
          ...
          <Show when={props.case.outcome}>
            <span class={`px-2 py-1 text-xs font-medium rounded-md border ${outcomeColors[props.case.outcome!]}`}>
              {props.case.outcome}
            </span>
          </Show>
        </div>
        ...
        <Show when={props.case.difficulties && props.case.difficulties.length > 0}>
          <div class="mt-3 pt-3 border-t border-slate-700">
            <span class="text-xs text-amber-400 font-medium flex items-center gap-1">
              ...
              {props.case.difficulties!.length} challenge{props.case.difficulties!.length > 1 ? 's' : ''} documented
            </span>
          </div>
        </Show>
      </div>
    ...
    >
      ...
      <div class="flex-1 p-4 flex flex-col justify-center">
        <div class="flex items-center gap-2 mb-2">
          ...
          <span class="text-slate-500 text-xs ml-auto">{props.formatDate(props.case.date)}</span>
        </div>
        ...
        <div class="flex items-center gap-4 mt-2 text-xs text-slate-500">
          ...
          <Show when={props.case.images.length > 0}>
            <span>📷 {props.case.images.length} image{props.case.images.length > 1 ? 's' : ''}</span>
          </Show>
          <Show when={props.case.difficulties && props.case.difficulties.length > 0}>
            <span class="text-amber-400">⚠️ {props.case.difficulties!.length} challenge{props.case.difficulties!.length > 1 ? 's' : ''}</span>
          </Show>
        </div>
      </div>
    </a>
